...
async def node_setup(state: BuilderState) -> dict:
    ...
    await execute_tool("run_command", {
        "command": f'git commit -m "feat: initial project generated by backend-builder"'
    }, project_name)
    await execute_tool("open_vscode", {"path": project_name}, project_name)

    logger.info("node_setup done project=%s files=%d", project_name, len(state.get("files_generated", [])))
    return {}
